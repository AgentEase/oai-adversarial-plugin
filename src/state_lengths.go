...
// Re-evaluate cached evidence when the configured acceptance policy changes.
func stateEntryAccepted(entry stateEntry) bool {
	return entry.Valid && isAcceptedStateLength(len(entry.Value))
}
