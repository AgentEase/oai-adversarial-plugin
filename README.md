...
- **轮换池（`# pool:` 标记，一个端点背后是大量可轮换源 IP，例如本地 IPv6 /64 动态代理）：永不冷却** —— 每次连接都会换一个新源地址，"这次抽到坏的"只反映当前上游状态，不代表池子坏了；失败计数继续累计（供面板观测），健康捕获清空计数，但不会把池子雪藏 3 小时。`exit-pool-fail-threshold` 仅为旧配置向后兼容保留，不再参与判定。
