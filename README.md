...
```
direct
socks5h://10.255.1.1:18321   # pool:ipv6
```
...
- **临近才预备**：当 active 的剩余有效期进入 `prefetch-minutes`（默认 3 分钟）窗口时，看门狗（每 30 秒巡检）才对该模型发起一次探测；每模型重试有 90 秒节流，已备好接班值时不重复；
- **平滑接手**：探测成功（模型一致 + 292 字节）的新值先停在接班槽；业务轨继续用旧 active 直到它自然到期，**到期瞬间原子接管**，用户体验上没有任何空窗或回退；
...
```
idle ──(剩余 TTL ≤ 预备窗口)──► probing
probing ──(验收通过: 模型一致 + 292B)──► 接班就绪（旧值继续服务，到期瞬间原子接管）
probing ──(单轮达到 max-attempts)──► failed（标注: 次数/轮数/最后错误/时间）
failed ──(下一次触发/成功获取)──► idle / store
```
