	...
	for _, target := range []string{"test-sol", "test-other-astra"} {
		for _, stream := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/stream=%t", target, stream), func(t *testing.T) {
				...
				} else if secondCapture.state != issuedState || second.Header.Get(turnStateHeader) != issuedState {
					t.Error("control must reproduce foreign state forwarding in both directions")
				}
			})
		}
	}
