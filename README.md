...
探测轮询会在出口间轮换；**未获得正确 state token 的出口**（长度异常、模型不一致或连接类错误）连续失败达到阈值后，被**短暂移出池子**并进入 `exit-cooldown-minutes`（默认 180 分钟）冷却，到期后自动回归；成功一次立即清除。两类阈值：

- **普通出口**：`exit-fail-threshold`（默认 3）次连续失败即冷却；
- **多出口池**（一个端点背后是大量可轮换源 IP，例如本地 IPv6 /64 动态代理）：`exit-pool-fail-threshold`（默认 **10**）次才冷却 —— 每次连接都会换一个源 IP，"这次抽到坏的"不代表池子坏了，应当优先再抽一次而不是雪藏 3 小时。
...
面板「代理池」标签页会把该出口显示为「**IPv6 池**」（或标记里声明的名称），策略卡片同时展示两类阈值。升级时自动做策略迁移：历史失败数低于新阈值的池出口立即解除冷却。
