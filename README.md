...
探测轮询会在出口间轮换；**未获得正确 state token 的普通出口**（长度异常、模型不一致或连接类错误）连续失败达到阈值后，被**短暂移出池子**并进入 `exit-cooldown-minutes`（默认 180 分钟）冷却，到期后自动回归；成功一次立即清除。两类出口：

- **普通出口（固定端点）**：`exit-fail-threshold`（默认 3）次连续失败即冷却；
- **轮换池（`# pool:` 标记，一个端点背后是大量可轮换源 IP，例如本地 IPv6 /64 动态代理）：永不冷却** —— 每次连接都会换一个新源地址，"这次抽到坏的"只反映当前上游状态，不代表池子坏了；失败计数继续累计（供面板观测），但不会把池子雪藏 3 小时。`exit-pool-fail-threshold` 仅为旧配置向后兼容保留，不再参与判定。
