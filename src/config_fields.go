...
var panelFields = []panelField{
	...
	{Name: "session-guard-mode", Type: "enum", EnumValues: []string{"off", "observe", "enforce"}, Description: "会话票据来源守卫：observe 只记录异账号回带；enforce 剥离已知异账号票据，并仅允许当前 AuthID 的健康票据接管。", path: []string{"session-guard-mode"}},
	{Name: "session-provenance-ttl-minutes", Type: "integer", Description: "会话到票据铸造账号的短期溯源有效期（1–1440 分钟）；不保存票据正文。", path: []string{"session-provenance-ttl-minutes"}, min: 1, max: 1440},
